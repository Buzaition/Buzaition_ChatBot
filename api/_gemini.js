...
export function getHealth() {
  ...
  return {
    ...
    model: GEMINI_MODEL,
    keys: keys.length,
    message: keys.length ? `Ready with ${keys.length} key(s)` : "Missing GEMINI_API_KEY"
  };
}
...
export async function generateChat(messages) {
  ...
  if (!keys.length) {
    return {
      status: 500,
      body: {
        error: "Missing GEMINI_API_KEY. Add one or more keys to Vercel environment variables."
      }
    };
  }
